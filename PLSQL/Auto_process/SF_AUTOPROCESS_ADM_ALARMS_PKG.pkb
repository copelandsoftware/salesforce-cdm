...
BEGIN
	...
	--chandra sep,2018 salesforce update
	log_file := UTL_FILE.FOPEN(directory_name,'SF_AUTOPROC_ADM'||'.log','a');
...
PROCEDURE check_auto_email_alarms (p_alm_id            IN     NUMBER,
   ...
   IS
     CURSOR rc_auto_email_rules
       IS
       SELECT AUTO_EMAIL_DESCRIPTION,sf_message_id
       FROM sf_message_config mmc
       WHERE mmc.sf_cust_id = p_cust_id
   ...
   BEGIN
    o_auto_email_flag :='N';
    FOR r_auto_email_rule IN rc_auto_email_rules
      LOOP
        FOR include_desc in (
          SELECT
        ...
        LOOP
          IF UPPER(include_desc.ex_d) = 'ALL' OR include_desc.ex_d IS NULL OR instr(UPPER(p_desc),UPPER(include_desc.ex_d))>0 THEN
            o_auto_email_flag :='Y';
          END IF;
	    ...
	    */
         ...
         END IF;
	  	...
	  	DBMS_OUTPUT.PUT_LINE ( ' Begnining of Check emails ' || lt_alarm_id(i) );
            ...
            IF g_processed_flag(g_alarm_count) ='P' AND v_adm_flag !='Y' AND v_auto_email_alarm_flag ='Y' THEN
