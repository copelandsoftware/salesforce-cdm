   ...
   WHERE
    ...
    CURSOR cur_tasks_alarm( p_evt_type VARCHAR2) IS
      ...
      lc_delete_date DATE :=trunc(sysdate)-6;
       ...
       --SF_ADM_REQ_LOG
      adm_req_delete_date DATE :=trunc(sysdate)-7;

      sf_wo_delete_date DATE := trunc(sysdate)-15;
...
BEGIN
    ...
    --delete SF_EXT_COMM_REQ_LOG
   	DELETE FROM
   			SF_EXT_COMM_REQ_LOG serl
   	WHERE   serl.CREATED_ON < sf_wo_delete_date;

    --delete work order for Meijer HQ
    DELETE FROM sf_work_order
    WHERE
    ext_wo_status IN (
        'SERVICE_COMPLETE',
        'REJECTED'
    )
    AND created_on < sf_wo_delete_date
    AND sf_site_id IN (
        SELECT
            ss.sf_site_id
        FROM
    ...
    DELETE FROM sf_work_order
    WHERE
    last_action = 'Cancel WorkOrder'
    and ext_wo_status is null
    AND created_on < sf_wo_delete_date
    AND sf_site_id IN (
        SELECT
            ss.sf_site_id
    ...
    --delete work order for Supervalu
    DELETE FROM sf_work_order
    WHERE
    created_on < sf_wo_delete_date
    AND sf_site_id IN (
        SELECT
            ss.sf_site_id
        FROM
        ...
        WHERE
            ss.sf_cust_id = sc.sf_cust_id
            AND sc.sf_cust_name = 'Supervalu'
    );

     --delete SF_SITE_LAST_COMMUNICATION_Q
   	DELETE FROM
   			 SF_ADM_REQ_LOG sarl
   	WHERE   sarl.CREATED_ON < adm_req_delete_date;
