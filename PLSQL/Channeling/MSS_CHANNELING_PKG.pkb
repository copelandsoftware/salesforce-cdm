create or replace PACKAGE BODY         MSS_CHANNELING_PKG AS
 ...
 PROCEDURE P_Get_Sitemapping(    p_site_name    IN VARCHAR2
   ...
   AND    sdm.RECORD_TYPE_NAME = 'Site_Mapping'
 ...
 (SELECT
   ...
   PROCEDURE p_get_alarm_flow_MSS_SF( p_cust_id IN NUMBER
    ...
    from
    SF_CONTROL_SYS
    where sf_site_id =v_sf_site_id and category is null and upper(IS_ACTIVE)='TRUE';
        ...
        FOR kk IN lt_alarm_id.FIRST..lt_alarm_id.LAST LOOP
         IF lt_sf_alarm_flow(kk) in ('SF','Both') then
           MERGE INTO SF_SITE_LAST_COMMUNICATION MSLC USING
                    (SELECT lt_sf_cust_id(kk) AS SF_CUST_ID,lt_sf_site_id(kk) AS SF_SITE_ID,lt_sf_asset_id(kk) AS SF_ASSET_ID,
                    TO_DATE(To_Char(Sysdate,'MMDD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') as TIME_RECEIVED,
                    ...
                    WHEN NOT MATCHED THEN
                    INSERT(SF_CUST_ID,SF_SITE_ID,SF_ASSET_ID,LAST_MSS_ALARM_ID,LAST_RECEIVED_DATE,LAST_RECEIVED_DATE_UTC,SITE_STATUS,created_on,created_by,modified_on,modified_by)
                    VALUES(ALARM.SF_CUST_ID,ALARM.SF_SITE_ID,ALARM.SF_ASSET_ID,ALARM.ALARM_ID,ALARM.TIME_RECEIVED,ALARM.TIME_RECEIVED_UTC,ALARM.SITE_STATUS,currentGMTTime,'admin',currentGMTTime,'admin');
         END IF;
        END LOOP;

        FOR kk IN lt_alarm_id.FIRST..lt_alarm_id.LAST LOOP
         IF lt_sf_alarm_flow(kk) in ('SF') then
           IF lt_descr(kk) not in ('Lost connectivity') THEN
                    INSERT INTO SF_SITE_LAST_COMMUNICATION_Q(SF_CUST_ID,SF_SITE_ID,SF_ASSET_ID,LAST_MSS_ALARM_ID,LAST_RECEIVED_DATE,LAST_RECEIVED_DATE_UTC,SITE_STATUS,created_on,created_by,modified_on,modified_by,processed_flag)
                    VALUES(lt_sf_cust_id(kk),lt_sf_site_id(kk),lt_sf_asset_id(kk),lt_alarm_id(kk),TO_DATE(To_Char(Sysdate,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS'),currentGMTTime,lt_sf_site_status(kk),currentGMTTime,'admin',currentGMTTime,'admin','YQ');
           END IF;
         END IF;
        END LOOP;
