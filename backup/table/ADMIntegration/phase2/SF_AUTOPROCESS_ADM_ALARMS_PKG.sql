create or replace PACKAGE BODY             SF_AUTOPROCESS_ADM_ALARMS_PKG
AS
   /*=========================================================================================
   ...
   g_site_condition_alarm_flag    g_varchar_type;
...
BEGIN
	...
	--chandra sep,2018 salesforce update
	log_file := UTL_FILE.FOPEN(directory_name,'SF_AUTOPROC'||'.log','a');
            ...
            WHERE
                ...
                AND NVL(sf_CUST_LEVEL_ID,-2) IN
                 ...
                 chandra sep,2018 salesforce update end*/
                    ...
                    --As per CR 20244,Added By Mritunjay on 29-March-2012
                    AND UPPER (mmc.approved_ind) NOT IN ('N', 'R') --As per CR 22123,Added By Mritunjay on 30-August-2012
                    --Added by Mritunjay Sinha on 28-August-2012 as implementing CB maintenace functionlity in MSSR
                    AND UPPER (
                          NVL (mmc.source_type, NVL (p_field14, 'AXBYWZ'))
                       ) = UPPER (NVL (p_field14, 'AXBYWZ'))
                    AND p_time_revd BETWEEN get_timezones_GMT_TO_SERVER (mmc.start_date)
                                        AND  get_timezones_GMT_TO_SERVER(NVL (
                                                              mmc.end_date,
                                                              (p_time_revd + 1)
                                                           ))
   ...
   BEGIN
      ...
      SELECT   mna.alarm_id
          ...
          WHERE
                  ...
                  --
                  AND p_time_revd BETWEEN get_timezones_GMT_TO_SERVER (r_rep_rec.start_date)
                                      AND  get_timezones_GMT_TO_SERVER(NVL (
                                                            r_rep_rec.end_date,
                                                            (p_time_revd + 1)
                                                         ))
      ...
      IS
         SELECT   email_delivery_id
          ...
          WHERE   maed.sf_cust_id = p_cust_id
                  ...
                  AND p_time_revd BETWEEN get_timezones_GMT_TO_SERVER (maed.start_date)
                                      AND  get_timezones_GMT_TO_SERVER(NVL (
                                                            maed.end_date,
                                                            (p_time_revd + 1)
                                                         ));
...
21-August-2012
      ...
      SELECT   mna1.alarm_id,
                  ...
                  FROM
                         (select * from (
                            ...
                            AND p_time_revd BETWEEN get_timezones_GMT_TO_SERVER (
                                                    start_date
                                                 )
                                             AND  get_timezones_GMT_TO_SERVER(NVL (
                                                                   end_date,
  ...
  PROCEDURE check_site_condition_alarms (
                                         p_cust_id IN VARCHAR2,
                                         p_site_id IN VARCHAR2,
                                         p_routing_group IN VARCHAR2,
                                         o_return_status OUT VARCHAR2,
                                         o_site_condition_flag OUT VARCHAR2
                                         )
  IS
  v_sc_config VARCHAR2(2):='N';
  v_sc_queue_config VARCHAR2(2):='N';
  CURSOR rc_site_config
  IS
  SELECT SITE_CONDITION_ENABLED
  FROM
  SF_SITE
  WHERE sf_site_id=p_site_id;

  CURSOR rc_cust_config
  IS
  SELECT SITE_CONDITION_ENABLED
  FROM
  SF_CUSTOMER
  WHERE sf_cust_id=p_cust_id;

  CURSOR rc_queue_config
  IS
  SELECT IS_QUEUE_SC_ENABLED,sf_queue_name
  FROM
  SF_SITE_CONDITION_CONFIG
  WHERE sf_queue_name =p_routing_group;
  BEGIN
    o_site_condition_flag:='N';
    FOR c_site_config in rc_site_config LOOP
      IF c_site_config.SITE_CONDITION_ENABLED='true' then
      v_sc_config:='Y';
      END IF;
    END LOOP;
    IF v_sc_config='N' THEN
      FOR c_cust_config in rc_cust_config LOOP
        IF c_cust_config.SITE_CONDITION_ENABLED='true' then
        v_sc_config:='Y';
        END IF;
      END LOOP;
    END IF;
    IF v_sc_config='Y' THEN
      FOR c_queue_config in rc_queue_config LOOP
        IF c_queue_config.IS_QUEUE_SC_ENABLED ='true' and c_queue_config.sf_queue_name is not null THEN
        v_sc_queue_config:='Y';
        END IF;
      END LOOP;
    END IF;
    IF v_sc_config='Y' AND v_sc_queue_config='Y' THEN
      o_site_condition_flag:='Y';
    END IF;
   o_return_status := 'S';
   EXCEPTION
      WHEN OTHERS
      THEN
        o_return_status := 'E';
        DBMS_OUTPUT.put_line ('INFO :- check_site_condition_alarm' || SUBSTR(SQLERRM,1,1600));
     g_info_msz :=   'INFO :- check_site_condition_alarm' || SUBSTR(SQLERRM,1,1600);

          WRITE_LOG_FILE(file_name => v_file_name
                              ,info     => g_info_msz
                              ,o_return_status  => v_return_status
                     ) ;
  END check_site_condition_alarms;
  ...
  PROCEDURE check_email_alarms (p_alm_id            IN     NUMBER,
   ...
   SELECT   'EMAIL FOUND' AS c1,maed.EMAIL_DELIVERY_ID
          ...
          WHERE       maed.sf_cust_id = p_cust_id
                  ...
                  AND p_created_on BETWEEN maed.start_date
                                      AND  NVL (
                                                            maed.end_date,
                                                            (p_created_on + 1)
                                                         );
    ...
    where
 	...
 	and is_exclude = 'true'
 	AND p_created_on BETWEEN smc.start_date AND  NVL ( smc.end_date, (p_created_on + 1) );
...
PROCEDURE check_mobile_alarms (  p_alm_id            IN     NUMBER,
   ...
   IS
      ...
      IS
         ...
         (
          ...
          AND p_time_revd BETWEEN get_timezones_GMT_TO_SERVER (mmc.start_date)
          AND get_timezones_GMT_TO_SERVER(NVL (mmc.end_date,(p_time_revd + 1))));
   ...
   BEGIN
      ...
      LOOP
        ...
        FOR exclude_source in (
              ...
              DBMS_OUTPUT.put_line (exclude_source.ex_s);
        ...
        IF v_source_adm_flag = 'N' AND v_descr_adm_flag = 'N' THEN
          o_adm_flag := 'N';
        END IF;
        IF v_source_adm_flag = 'N' AND r_adm_rule.DESCRIPTION_EXCLUDE IS NULL THEN
          o_adm_flag := 'N';
        END IF;
        IF v_descr_adm_flag = 'N' AND r_adm_rule.source_exclude IS NULL THEN
          o_adm_flag := 'N';
        END IF;
      ...
      IF o_adm_flag = 'Y' THEN
		FOR r_adm_ad_rule IN rc_adm_ad_rules
		  LOOP
			...
			v_source_adm_ad_null_flag := NULL;
			v_descr_adm_ad_null_flag := NULL;
        ...
        DBMS_OUTPUT.put_line ('starting procedure chk_adm_AD_alarms-1'||v_source_adm_ad_flag);
			...
			FOR include_description in (
			  ...
			  v_descr_adm_ad_null_flag :='N';
			  IF instr(UPPER(p_desc),UPPER(include_description.inx_d))>0 THEN
				v_descr_adm_ad_flag :='Y';
				EXIT WHEN v_descr_adm_ad_flag = 'Y';
			  END IF;
			END LOOP;
      DBMS_OUTPUT.put_line ('starting procedure chk_adm_AD_alarms-2'||v_descr_adm_ad_flag);

			IF v_source_adm_ad_null_flag IS NULL OR v_source_adm_ad_null_flag ='N' THEN
			   v_source_adm_ad_flag := 'Y';
			END IF;

			IF v_descr_adm_ad_null_flag IS NULL OR v_descr_adm_ad_null_flag='N' THEN
			   v_descr_adm_ad_flag := 'Y';
			END IF;
  ...
  PROCEDURE update_alarms_statuses_temp (p_alarm_id IN NUMBER,
   ...
   SET
   	...
   	--chandra oct,2018 salesforce change
   ...
   BEGIN
      ...
      FORALL i IN 1 .. g_alarm_count
         UPDATE   sf_norm_alarm mna
            SET   mna.request_id = -100,
                  ...
                  -- CURRENT_STATUS     =DECODE(g_processed_flag(i),'Y','Resolved',NULL) -- Added By Mritunjay Sinha on 20Feb2012
                  --chandra Feb,14,2012 xian is going to use this field so commented
                  --email_processed    = g_email_processed(i),
                  --email_alert_id     = g_alert_id(i)
                  time_available_process =
                     DECODE (g_processed_flag (i),
                             'P', g_time_available_process (i),
                             time_available_process), --Added By Mritunjay Sinha on 13 June2012 for CR# 21142
                  --chandra sep 2018 salesforce update
                  sf_routing_group = g_routing_group (i),
          --Added by Mritunjay Sinha on 28-August-2012 as implementing CB maintenace functionlity in MSSR  as per CR #22163
                  email_processed=DECODE(g_email_alarm_flag(i),'Y','Y','N'),
                  SITE_CONDITION_FLAG=DECODE(g_site_condition_alarm_flag(i),'Y','YQ','N'),
      ...
      --Added by mritunjay on 27feb2012
      FOR i IN 1 .. g_almactstat_count
      LOOP                     --chandra june,21,2012 used the global variable
         UPDATE   sf_norm_alarm mna
      ...
      -- This loop will run only for all the auto processed alarms for which the service_Request_id
      -- is created.
       /* chandra sep, 2018 salesforce update begin
      FOR i IN 1 .. g_almactstat_count
      LOOP                     --chandra june,21,2012 used the global variable
         UPDATE   sf_norm_alarm mna
            SET   sr_reference =null --chandra, oct,2018 salesforce commented g_service_request_id (i)
          WHERE   mna.alarm_id = g_almactstat_alm_id (i);
      END LOOP;



      --Below piece of code is added by Mritunjay Sinha to populate the CURRENT_ALARM_ACTION_ID,LAST_ALARM_ACTION_ID column of MSS_NORM_ALARM
      FOR i IN 1 .. g_almactstat_count
      LOOP                     --chandra june,21,2012 used the global variable
         UPDATE   sf_norm_alarm mna
            SET   current_alarm_action_id = g_alarm_action_id (i),
                  last_alarm_action_id = g_alarm_action_id (i)
          WHERE   mna.alarm_id = g_almactstat_alm_id (i);
      END LOOP;

      chanda sep,2018 salesforce update end */
      ...
      LOOP
         ...
         g_time_available_process (i) := NULL;          --chandra june,21,2012
      ...
      g_alarm_count := 0;
      --chandra june,21,2012 used the global variable
      g_almactstat_count := 0;
      o_return_status := 'S';
    EXCEPTION
      WHEN OTHERS
   ...
   PROCEDURE validate_alarm_proc (errbuf OUT VARCHAR2, retcode OUT VARCHAR2)
   IS
      /*===========================================================================
      ...
      --chandra sep,2018 salesforce related update
      /* Cursor for fetching the alarm details from the XXCS_RAW_UARD */
      CURSOR cur_alm_data
      IS
           SELECT
                    ALARM_ID,
                    ...
                    routing_group, --Added by Mritunjay Sinha on 28-August-2012 as implementing CB maintenace functionlity in MSSR
                    rtn_date,
                    FIELD5,
                    FIELD6,
                    FIELD8,
   ...
   BEGIN
    ...
    FOR i IN lt_alarm_id.FIRST..lt_alarm_id.LAST LOOP
         ...
         --chandra, Aug,21 moved the test logic to the top
	...
	/* chandra May 21 2020 commented start

         dbms_output.put_line(' Check email procedure ');
         check_email_address (p_alarm_id             => lt_alarm_id(i),
                              p_cust_id              => lt_cust_id(i),
                              p_site_id              => lt_site_id(i),
                              p_time_revd            => lt_time_received(i),
                              o_email_delivery_id    => v_email_alert_id,
                              o_email_address_flag   => v_email_address_flag,
                              o_return_status        => v_return_status,
                              o_err_msg              => v_err_msg);

         dbms_output.put_line(' Check email id ' || v_email_alert_id);

         IF v_return_status = 'E'
         THEN
            --chandra, Aug,21
            log_error_table (i_alm_id       => lt_alarm_id(i),
                             i_error_name   => 'Check Email Address',
                             i_error        => v_err_msg);
         --chandra, Aug,21 dbms_output.put_line('Error after Check_Email_address procedure.');
         -- RAISE v_manage_alm_exc;
         END IF;

	chandra May 21 2020 commented end */
         ...
         THEN
            chk_disregard_alarms (
               ...
               p_time_revd             => lt_time_received(i),
               p_adv_type              => lt_field8(i), --As per CR 20244,Added By Mritunjay on 29-March-2012
               p_adv_value             => lt_field6(i), --As per CR 20244,Added By Mritunjay on 29-March-2012
			   p_prop_name             => lt_field5(i), --As per SPA1873,Added By Ivy on 12-NOV-2020
    ...
    IF NOT v_auto_process_flag
         ...
         --Check mobile Rule
            check_mobile_alarms (
               ...
               o_mobile_flag           => v_mobile_flag,
               o_mobile_alert_only     => v_mobile_alert_only,
               o_mobile_alert_only_inactive => v_mobile_alert_only_inactive,
               o_no_mobile_rule_flag   => v_no_mobile_rule_flag,
               o_no_active_mobile_rule_flag => v_no_active_mobile_rule_flag,
               o_mobile_service_level  => v_mobile_service_level,
               o_return_status         => v_return_status
            );
            ...
            THEN
               log_error_table (i_alm_id       => lt_alarm_id(i),
                                i_error_name   => 'Check mobile Found',
                                i_error        => v_err_msg);
            END IF;

            IF v_mobile_flag = 'Y' AND v_mobile_alert_only is NULL
            THEN
               v_auto_process_flag := TRUE;
               store_alarm_status_new (
                  i_alm_id            => lt_alarm_id(i),
           ...
           IF v_mobile_alert_only = 'Y' THEN
               v_auto_process_flag := TRUE;
               IF v_mobile_flag = 'Y' THEN
                store_alarm_status_new (
                  i_alm_id            => lt_alarm_id(i),
                  i_cust_id           => lt_cust_id(i),
                  i_site_id           => lt_site_id(i),
                  ...
                  i_email_alert_id    => v_email_alert_id --store alarm action, status, notes
                                                         ,
                  i_action_id         => g_mobile_alert_action_id,
                  i_status_id         => g_mobile_alert_status_id,
                  i_site_contact_id   => NULL,
                  i_notes             => 'Alert only alarm is resolved automatically',
                  i_current_status    => 'Resolved',
                  i_time_available    => lt_created_on(i),
                  i_routing_group     => lt_routing_group(i),
                  o_return_status     => v_return_stat
               ...
               ELSE
                store_alarm_status_new (
                  ...
                  i_status_id         => g_mobile_alert_status_id,
                  i_site_contact_id   => NULL,
                  i_notes             => 'Alarm is resolved due to not pass the Mobile Rule',
                  i_current_status    => 'Resolved',
                  i_time_available    => lt_created_on(i),
                  i_routing_group     => lt_routing_group(i),
                  o_return_status     => v_return_stat
          ...
          IF v_mobile_alert_only_inactive = 'Y' THEN
             v_auto_process_flag := TRUE;
             store_alarm_status_new (
                  i_alm_id            => lt_alarm_id(i),
                  i_cust_id           => lt_cust_id(i),
                  ...
                  i_status_id         => g_mobile_alert_status_id,
                  i_site_contact_id   => NULL,
                  i_notes             => 'Alarm is resolved due to mobile alert service is inactive.',
                  i_current_status    => 'Resolved',
                  i_time_available    => lt_created_on(i),
                  i_routing_group     => lt_routing_group(i),
                  o_return_status     => v_return_stat
          ...
          --send email
          IF v_mobile_service_level IS NOT NULL THEN
             IF v_no_mobile_rule_flag = 'Y' THEN
                v_mobile_rule_send_mail_flag := 'Y';
                v_mobile_rule_mail_subject := 'Mobile Rule Does Not Exist';
                IF v_mobile_service_level = 'Mobile Alert' THEN
                   v_mobile_rule_mail_body := 'Alarm was resolved and not sent to mobile user, because Mobile rule does not exist.';
                ELSE
                   v_mobile_rule_mail_body := 'Alarm sent to MSSR Queue and not sent to mobile user, because Mobile rule does not exist.';
                END IF;
             END IF;

             IF v_no_active_mobile_rule_flag = 'Y' THEN
                v_mobile_rule_send_mail_flag := 'Y';
                v_mobile_rule_mail_subject := 'No Active Mobile Rule Exist';
                IF v_mobile_service_level = 'Mobile Alert' THEN
                   v_mobile_rule_mail_body := 'Alarm was resolved and not sent to mobile user, because Mobile rule is not active.';
                ELSE
                   v_mobile_rule_mail_body := 'Alarm sent to MSSR Queue and not sent to mobile user, because Mobile rule is not active.';
                END IF;
             END IF;

            IF v_mobile_rule_send_mail_flag = 'Y' THEN
              select ref_name into v_site_name from cmn_reference where ref_id=lt_site_id(i);
              SF_SEND_MAIL_PKG.send_mail
               (
                    p_recipient => v_emailAdd_mobilealert,
                    p_subject => v_mobile_rule_mail_subject,
                    p_message =>'<html>
                              <p>'||v_mobile_rule_mail_body||'</p>
                              <p>Please check the mobile filter rule for site: '||v_site_name||'</p>
                              <p>-------------------------------------------------------------------------------------------</p>
                              <table>
                              <tr><td colspan="2">Alarm info: </td> </tr>
                              <tr><td>Alarm ID: </td><td> '|| lt_alarm_id(i) ||'</td></tr>
                              <tr><td>Site Name: </td><td> '|| v_site_name ||'</td></tr>
                              <tr><td>Raw Source: </td><td>'|| lt_source(i) ||'</td></tr>
                              <tr><td>Raw Description: </td><td>'|| lt_descr(i) ||'</td></tr>
                              <tr><td>Time Received: </td><td>'||to_char(lt_time_received(i),'MM/DD/YYYY hh24:mi:ss') || '</td></tr>
                              <tr><td>Time Occurred: </td><td>'||to_char(lt_time_occurred(i),'MM/DD/YYYY hh24:mi:ss') || '</td></tr>
                              </table>
                              </html>'
               );
             END IF;
	  ...
	  chandra sep, 2018 salesforce commented, since mobile is available through community end */
	 ...
	 DBMS_OUTPUT.PUT_LINE ( ' Begnining of Snooze ' || lt_alarm_id(i) );
         ...
         THEN
            ...
            /* chandra sep,2018 salesforce commented needs to reviewed and enable based on user story

            --validating for a production alarm any snooze rule is defined for its customer
            snooze_alarms (p_alarm_id          => lt_alarm_id(i),
                           p_cust_id           => lt_cust_id(i),
                           p_descr             => lt_descr(i),
                           o_snooze_time_min   => v_snooze_time_min, -- chandra june,21,2012
                           o_return_status     => v_return_status);

		chandra sep,2018 salesforce enable after snooze is defined end
	    */
            ...
            /*  update MSS_NORM_ALARM
              SET time_available_process = v_snooze_time
              where alarm_id = lt_alarm_id ;*/
            ELSE
               store_alarm_status_new (
                  i_alm_id            => lt_alarm_id(i),
                  i_cust_id           => lt_cust_id(i),
         ...
         DBMS_OUTPUT.PUT_LINE ( ' Begnining of Check site condition alarms ' || lt_alarm_id(i) );
            --Check site condition alarms
          IF NOT v_auto_process_flag AND NOT v_threshold_flag THEN
            check_site_condition_alarms (
               p_cust_id               => lt_cust_id(i),
               p_site_id               => lt_site_id(i),
               p_routing_group         => lt_routing_group(i),
               o_return_status         => v_return_status,
               o_site_condition_flag   => v_sc_alarm_flag
            );
         g_site_condition_alarm_flag(g_alarm_count):=v_sc_alarm_flag;
         ELSE
         g_site_condition_alarm_flag(g_alarm_count):='N';
         END IF;
...
END SF_AUTOPROCESS_ADM_ALARMS_PKG;