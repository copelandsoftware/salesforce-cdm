...
--DROP IF JOB EXISTS.
	select COUNT(*) INTO job_exists from user_scheduler_jobs 
	    where job_name = 'SF_DG_INNOVATIONS_JOB';
	IF job_exists >= 1
	    THEN dbms_scheduler.drop_job(job_name => 'SF_DG_INNOVATIONS_JOB');
	END IF;
--CREATE JOB
    dbms_scheduler.create_job
    (job_name => 'SF_DG_INNOVATIONS_JOB',
    ...
    job_action=> 'DECLARE
				  ...
				  BEGIN
					  ...
					  SF_DG_INNOVATIONS_PKG.DG_INNOVATIONS_PROC(
						ERRBUF => ERRBUF,
						RETCODE => RETCODE
					  );
				  END;',
